...
export async function POST(
  ...
  try {
    ...
    if (!userId) {
      ...
      userId = activeUsers.data[0]?.id || 'guest-user';
    }
    ...
    const result = await AssessmentService.recordProctorViolation(
      assessmentId,
      userId,
      body.violationType || 'Tab switch detected',
      clientIp,
      userAgent
    );
    ...
    return NextResponse.json(
      {
        error: 'Failed to record proctor violation',
